...
홈의 전체 냉장고는 **재료명·카테고리 검색**, **전체/임박/폐기 필터**, 상태별 개수를 제공하며, 생성된 레시피는 버튼 한 번으로 복사할 수 있습니다.
...
1. 서버가 CSV·기준일·파이프라인 버전과 저장 캐시를 확인합니다. 모두 같을 때만 Agent 1 결과를 재사용하며, 날짜가 바뀌면 D-day 결과를 자동으로 다시 만듭니다.
...
| | 코드 검증 | Agent 3 호출 전에 제목·필수 섹션·조리 5~7단계·임박 재료 포함·폐기 재료 미사용을 Python이 결정적으로 검사 |
| | 입력 경계·우선순위 | 재고와 외부 검색 결과를 태그로 분리하고, 안전 → 재고 일치 → 타입 정체성 → 형식 순으로 충돌 해결 우선순위를 명시 |
| | Few-shot + Self-check | 타입별 좋은/피해야 할 메뉴명 예시를 1쌍씩 주고, 최종 답변 전 제약·형식을 내부 점검하되 점검 과정은 출력하지 않도록 지시 |
...
| **Agent 4** 장보기 관리사 | **ReAct 응용** | Observe(재고 확인) → Decide(보충 판단) → Act(선택 플랫폼 링크 변환). 중간 추론은 숨기고 최종 결과만 출력 |
...
├── MVP_IMPROVEMENT_REVIEW.md # MVP 불편점·반영 내용·후속 개발 백로그
└── README.md
```
...
| 수업 개념 | 본 프로젝트 적용 |
|---|---|
| **Inference 파이프라인 / 다단계 호출 비용** | 브리핑 A는 **1회만 계산** 후 모든 후속 Task에 텍스트로 주입. 모든 결과는 `nbax_data.js`에 영속 캐시 (CSV·기준일·파이프라인 변경 시 자동 무효화) → 중복 호출 비용 절감 |
...
| **ReAct 응용** | Agent 4가 Observe(재고 확인) → Decide(보충 판단) → Act(선택 플랫폼 링크 변환) 절차로 동작하며 중간 추론은 출력하지 않음 |
